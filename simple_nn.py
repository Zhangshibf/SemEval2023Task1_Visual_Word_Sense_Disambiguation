...
def train_one_epoch(model,device,dataloader,optimizer):
    text_encoder = CLIPTextModelWithProjection.from_pretrained("openai/clip-vit-base-patch32").to(device)
    image_encoder = CLIPVisionModelWithProjection.from_pretrained("openai/clip-vit-base-patch32").to(device)
    loss_total = 0
    correct_total = 0
    instance_total =0
    ...
    for keywords,contexts,augmentations,image_names,image_paths in dataloader:
        ...
        text_outputs = self.text_encoder(tokens).text_embeds
        image_outputs = self.image_encoder(images).image_embeds
#I'm here
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        train_model(model, device=device, epoch=int(args.epoch), path_train=args.train, path_out=args.output)
    elif args.mode == 'test':
